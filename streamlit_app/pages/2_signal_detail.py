...
search_queries  = (brief.get("search_queries") or [])       if brief else []
...
if not is_investigated:
    ...
    <div class="ms-pending-card">
        ...
        <div class="ms-pending-desc">
            This signal was flagged by the PRR pipeline but has not been run through the agent pipeline yet
        </div>
...
with left_col:
    ...
    # Search Queries (Agent 1 Generated)
    if search_queries:
        with st.expander("Search Strategy", expanded=False):
            st.markdown("""
            <div style="font-family:var(--font-mono); font-size:11px; color:var(--text-muted);
                        letter-spacing:0.5px; margin-bottom:12px;">
                PubMed search queries generated for literature retrieval:
            </div>
            """, unsafe_allow_html=True)
            ...
            for i, query in enumerate(search_queries, 1):
                ...
                <div style="background:var(--bg-elevated); border:1px solid var(--border);
                    ...
                    <div style="font-family:var(--font-mono); font-size:10px; font-weight:500;
